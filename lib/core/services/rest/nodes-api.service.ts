...
export class NodesApiServiceImpl implements NodesApiService {
    ...
    unlockNode(nodeId: string, opts?: any): Observable<NodeEntry> {
        return from(this.nodesApi.unlockNode(nodeId, opts));
    }

    requestDirectAccessUrl(nodeId: string): Observable<DirectAccessUrlEntry> {
    ...
    getNodeEntry(nodeId: string, options: any = {}): Observable<NodeEntry> {
        const defaults = {
    ...
    getNode(nodeId: string, options: any = {}): Observable<MinimalNode> {
        ...
        return this.getNodeEntry(nodeId, queryOptions).pipe(
